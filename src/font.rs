...
pub fn layout_numbers<'i: 'fd, 'fd>(
    ...
    for (digit, glyph) in digits.iter_mut().enumerate() {
        let the_glyph = font.font.glyph(Codepoint(digit as u32 + b'0' as u32));
...
pub fn ellipsis<'fd>(
    layout: impl Iterator<Item = PositionedGlyph<'fd>>,
    max_x: f32,
...
pub fn measure_default_layout(font: ScaledFont, text: &str) -> f32 {
